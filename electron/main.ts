...
function getPluginsDir(): string | null {
  if (!vaultPath) return null;
  const dir = path.join(vaultPath, ".plugins");
...
ipcMain.handle("plugins:load", async (_event, name: string) => {
  try {
    const dir = getPluginsDir();
    if (!dir) return "";
...
ipcMain.handle("plugins:save-file", async (_event, name: string, content: string) => {
  try {
    const dir = getPluginsDir();
    if (!dir) return false;
    fs.writeFileSync(path.join(dir, `${name}.js`), content, "utf-8");
    return true;
  } catch {
    return false;
  }
...
ipcMain.handle("plugins:delete-file", async (_event, name: string) => {
  try {
    const dir = getPluginsDir();
    if (!dir) return false;
    const jsPath = path.join(dir, `${name}.js`);
